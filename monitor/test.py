...
#cd /data/users/hwwang/app ; python check_servers.py
...
for server,port in server_list.items():
    res = subprocess.call(" /usr/local/bin/docker ps |  grep {} >/dev/null".format(server),shell=True)
    if res == 0 :
        pass
    else:
        data = {}
	...
	subprocess.call("cd  /usr/local/script && sh {}.sh start >/dev/null".format(server),shell=True)
	continue
    ...
    if nc == 0 :
        ...
        res = requests.post(url, json=data, headers=headers)
        print res.status_code
	subprocess.call("cd  /usr/local/script && sh {}.sh restart >/dev/null".format(server),shell=True)